...
async def cmd_detener(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if motor:
        motor.detener()
    await msg.reply_text("*Bot detenido*", parse_mode="Markdown")
...
async def cmd_operaciones(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ops:
        ...
        for i, op in enumerate(ops, 1):
            ...
            emoji  = "🔥" if pnl > op["tamaño_usdt"]*0.02 else ("⚠️" if pnl < -op["tamaño_usdt"]*0.015 else "📈" if pnl > 0 else "📉")
            lineas.append(
                f"{emoji} *{op['par']}*\n"
...
async def cmd_historial(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not trades:
        await msg.reply_text("Sin historial aún.")
        return
    lineas = [f"*Ultimos {len(trades)} trades*\n"]
    for t in trades:
        emoji = "✅" if t.pnl_usdt > 0 else "❌"
        lineas.append(
            f"{emoji} *{t.par}* | {t.pnl_usdt:+.2f} USDT\n"
            f"_{t.razon_salida}_ | {t.duracion_min} min\n"
        )
    await msg.reply_text("\n".join(lineas), parse_mode="Markdown")
...
async def cmd_modo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if modo not in modos:
        await update.message.reply_text("Modos: conservador, normal, agresivo")
        return
